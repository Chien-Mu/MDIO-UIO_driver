	...
	unsigned short regAddr)
{
	...
	printf("write cmd mii.data=0x%x\n", mii.data);
	...
	return (int)mii.data; //read data mii.data
}
	...
	unsigned short regAddr)
{
	...
	mii.reg   = regAddr;
	retVal = read(fd, (char*)&mii, sizeof(mii));
	//printf("read SMI reg devAddr: %X, regAddr: %X data = 0x%x\n", devAddr, regAddr,(int)mii.data);
	if(retVal < 0)
	{
		printf("Error: Accessing SMI reg devAddr: %X, regAddr: %X\n", devAddr, regAddr);
		return -1;
	...
	unsigned short data)
{
	...
	retVal = write(fd, (char*)&mii, sizeof(mii));
	//printf("write SMI reg devAddr: %X, regAddr: %X\n", devAddr, regAddr);
	if(retVal < 0)
	{
		printf("Error: Accessing SMI reg devAddr: %X, regAddr: %X\n", devAddr, regAddr);
		return -1;
